...
const addInteraction = async (req, res) => {
  try {
    ...
    if (String(userId) === String(post.user)) {
      return res
        .status(400)
        .send({ message: "You cannot interact with your own post" });
    }
    // Checks if the post is expired and stops interaction if it has
    if (post.status.includes("Expired")) {
      return res
        .status(403)
        .send({ message: "Post has expired, no longer able to interact" });
    }
    ...
    if (existingInteraction) {
      ...
      const updatedInteraction = await updateHelper.updateInteraction(
        post,
        existingInteraction,
        type,
      );
      ...
      const addedInteraction = await createHelper.createInteraction(
        post,
        postId,
        userId,
        type,
...
const deleteSpecificInteraction = async (req, res) => {
  try {
    ...
    if (String(userId) !== String(interaction.user)) {
      return res
        .status(403)
        .send({ message: "You are not authorized to delete this interaction" });
    }
