  ...
  try {
    ...
    if (
      ...
      return res.status(400).send({
        message:
          "Invalid title, must be a string and contain between 3 and 70 characters.",
      });
    }
    ...
    // which is then added by timesing by 1000 for the milliseconds it is measuted in and then timesing by 60
    // for the minutes
    ...
    const postToSave = await createHelper.createPost(
      req.body.title,
      req.body.topic,
      req.body.body,
      expiry_time,
...
const getPosts = async (req, res, filter = {}) => {
  // Filter is nothing by default so it will get everything
  try {
...
const getPostsByTopic = async (req, res) => {
  try {
    ...
    // is capitalised
    const topic =
      req.params.topic[0].toUpperCase() +
...
const updateExpirationTime = async (req, res) => {
  try {
    ...
    const expiry_minutes = req.body.expiry_minutes;
    ...
    if (!expiry_minutes && expiry_minutes !== -1) {
      return res
        .status(400)
        .send({ message: "New expiration time is required" });
    }
    ...
    if (!post.user.equals(userId)) {
      return res
        .status(403)
        .send({ message: "You are not authorized to update the post expiry" });
    }
    const expiry_time = new Date(Date.now() + expiry_minutes * 60 * 1000);
    ...
    if (!updatedPost) {
      return res
        .status(400)
        .send({ message: "Error updating expiration time" });
    }
